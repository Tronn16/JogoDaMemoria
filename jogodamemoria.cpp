...
{
    // Converte coordenadas do mouse para o espaço OpenGL normalizado [-1, 1]
    float normalizedX = (2.0f * x) / width() - 1.0f;
    float normalizedY = 1.0f - (2.0f * y) / height();

    // Ajustando os valores para corresponder ao layout real das cartas
    float cartaWidth = 0.38f;  // Largura entre as cartas
    float cartaHeight = 0.94f; // Altura entre as linhas de cartas
    float startX = -0.7f;      // Posição X inicial
    float startY = 0.8f;       // Posição Y inicial da primeira linha

    // Verifica se o clique está dentro da área vertical válida
    int linha = -1;
    ...
    // Determina a coluna baseado na posição X ajustada
    int coluna = -1;
    float adjustedX = normalizedX; // Mantemos o X original
    ...
    // Retorna -1 se o clique foi fora da área das cartas
    if (linha == -1 || coluna == -1)
    {
        return -1;
    }

    // Calcula o índice da carta
...
void inicializarCartas()
{
    ...
    for (int i = 0; i < 8; i++)
    {
        pares = false;
        int fig = rand() % 4;
        // O código abaixo serve para garantir que haverá apenas um par de cartas de cada figura
        while (!pares)
        {
            pares = true;
            if (QtdFig0 == 2 && fig == 0)
